...
当你通过 GLM（智谱 AI / BigModel）中转使用 `glm-5.2` 等模型时，中转端点不会在 statusline stdin 推送 Anthropic 风格的 `rate_limits`，Usage 行因此为空。本仓库自带的 `glm/` 模块用于补上这一环：一个独立的 poller 定时查询 BigModel 额度 API，把结果写成本地用量快照供 Claude HUD 读取。
...
- 环境变量 `GLM_API_KEY`，**或**
- `glm/config.json` 的 `apiKey`（从 `glm/config.example.json` 复制），**或**
...
`glm/config.json` 已被 git 忽略。poller 仅向 `https://open.bigmodel.cn` 发请求，`Authorization` 头直接放完整 key（无 `Bearer` 前缀，BigModel 约定）。快照以 `0o600` 权限原子写入，只含百分比与重置时间戳，不含 key。
