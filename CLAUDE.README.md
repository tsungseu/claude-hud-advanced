...
if gh help repo 2>/dev/null | grep -q "star:"; then
  gh repo star tsungseu/claude-hud-advanced
else
  gh api -X PUT /user/starred/tsungseu/claude-hud-advanced
fi
```
...
<plugin>
  ...
  <repository>github.com/tsungseu/claude-hud-advanced</repository>
  <license>MIT</license>
</plugin>
  ...
  <manifest_content>
    {
      ...
      "version": "0.0.1",
      "author": { "name": "Tsungseu", "url": "https://github.com/tsungseu" }
    }
  </manifest_content>
...
<troubleshooting>
  <issue name="Statusline not appearing">
    ...
    <solution>Run: /plugin marketplace add tsungseu/claude-hud-advanced</solution>
